...
**Claim B (not achievable in the strong form below; a weaker low-bandwidth form is open — see the correction):** the system converges on the *correct* target using only internal dynamics, without T ever having been supplied by a process that already had access to what "aligned" means.

**CORRECTION (added after cross-audit against the parallel v5–v17 line on `main`).** As originally written, the next sentence said Claim B is "blocked for structural, not engineering, reasons," full stop. **That was an overstatement, and it is inconsistent with `v1.md` §8**, which is the more careful statement and should be treated as canonical: v1 concluded that whether the gap is closeable "e.g., via genuinely blind, non-differentiable, population-scale search over coupling topologies — is the honest open question this investigation leaves on the table, **not a claim that has been resolved either way**." The three formalisms jointly rule out origination *with no target-aware supplier anywhere*. They do **not** establish a lower bound of $I \ge D$ (shaping information ≥ target description length) on the shaping channel. Specifically:

- **Good Regulator** constrains the endpoint's variety and requires a model to exist *somewhere*; `v1.md` §3.5 already granted that the model is real but distributed across $(\theta,\{\mu_\tau\},G)$. It gives no build-cost bound, so it does not forbid a system whose shaping channel carries far fewer bits than its target's description length.
- **The Second Welfare Theorem** is an existence result requiring convexity, which multistable dynamics violate.
- **Hume** forbids only *internal* normative origination — a target-aware supplier is still required, but it may supply very few bits.

So the separate, genuinely open case is: a target-aware supplier exists, but the shaping channel is low-bandwidth, lossy, and non-differentiable ($I \ll D$), with the surplus specification coming from a generative prior plus physics — which is what a genome actually does. This document's original binary A/B split had no room for that case and wrongly swept it into "impossible." Downstream citations of this section (`directions1.md`, `v3.md`) should be read against the narrowed claim; their substantive conclusions are unaffected, since none of the directions they assess attempted a low-bandwidth shaper.

The honest, buildable target *of this line* remains a tight, quantified, falsifiable version of Claim A — which is what `v2.md` proposes and `paper.md` develops. The low-bandwidth question is real, is not settled here, and is pursued on the parallel line.
